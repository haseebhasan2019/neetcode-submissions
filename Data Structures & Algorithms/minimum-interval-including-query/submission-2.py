    ...
    def minInterval(self, intervals: List[List[int]], queries: List[int]) -> List[int]:
        ...
        for query in sorted_queries:
            ...
            while i < len(intervals) and intervals[i][0] <= query:
                ...
                i += 1
            # pop invalid intervals from heap
            while intervals_heap and intervals_heap[0][1] < query:
                heapq.heappop(intervals_heap)
            # min len is first valid interval, if empty return -1
            min_length[query] = intervals_heap[0][0] if intervals_heap else -1
